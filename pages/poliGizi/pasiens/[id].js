...
function useGlobalPilihPasienPoliGizi() {
  return useSelector((state) => state.pilihPasienPoliGizi, shallowEqual);
}
const DetilPasien = ({ id, authorization }) => {
  ...
  const pilihPasien = useGlobalPilihPasienPoliGizi();
  ...
  useEffect(() => {
    if (pilihPasien != null) {
      if (pilihPasien.poli_gizi != null) {
        console.log(pilihPasien);
        if (pilihPasien.rawat_inap != null) {
          setFormValue({
            ...formValue, ['diagnosis']: '',
            ['resep_obat']: '',
            ['diagnosis_pengobatan']: pilihPasien.poli_gizi.diagnosis_pengobatan,
            ['nama_rujuk']: ''
          })
          setErrorLabel({
          ...
          setFormValue({
            ...formValue, ['diagnosis']: pilihPasien.poli_gizi.diagnosis,
            ['resep_obat']: pilihPasien.poli_gizi.resep_obat,
            ['diagnosis_pengobatan']: pilihPasien.poli_gizi.diagnosis_pengobatan,
            ['nama_rujuk']: ''
          })
          setErrorLabel({
  ...
  const ubahDataPoliGizi = () => {

    axiosClient.post('Poligizi/ubahDataPoliGizi',
      {
        id: pilihPasien.id,
        toggle: toggleMenu,
        ...
        Swal.close();
        dispatch({
          type: 'STORE_DATA_POLI_GIZI',
          allDataPoliGizi: data.allDataPoliGizi.data,
          pilihPoliGizi: data.allDataPoliGizi.data[id]
        })
        Swal.fire({
          icon: "success",
          title: "Berhasil",
  ...
  const hapusDataPoliGizi = () => {
    Swal.showLoading();
    axiosClient.post('Poligizi/hapusDataPoliGizi',
      {
        id: pilihPasien.id,
        diagnosis_pengobatan: formValue.diagnosis_pengobatan,
        diagnosis: formValue.diagnosis,
  ...
  const tambahDataPoliGizi = () => {
    axiosClient.post('Poligizi/tambahDataPoliGizi',
      {
        id: pilihPasien.id,
        toggle: toggleMenu,
        diagnosis_pengobatan: formValue.diagnosis_pengobatan,
        ...
        <div className="d-block">
          ...
          {pilihPasien != null ? (
            <div className=" w-75 d-inline-block align-top mt-5 mb-2" style={styleContainer}>
              <div className="mt-2">
                <h1 className="w-100 text-center">Form Data Pasien Poli Gizi</h1>
                ...
                <div className="d-inline-block ps-2 align-top"
                  style={styleBox}>
                  <h3 className="text-capitalize " style={{ borderBottom: "2px solid grey" }}>Form Data Poli Gizi</h3>
                  ...
                  <div >
                    {
                      pilihPasien != null ? (
                        pilihPasien.poli_gizi == null ? (
                          <button className="btn btn-primary" onClick={tambahDataPoliGizi}>Tambah Data Poli Gizi</button>
                        ) : (
                          <div>
                            <div className="w-25 d-inline-block me-2" >
                              <button className="btn btn-success w-100" onClick={ubahDataPoliGizi}>Ubah Data Poli Gizi</button>
                            </div>
                            <div className=" d-inline-block" style={{ width: "30%" }} >
                              <button className="btn btn-danger w-100" onClick={hapusDataPoliGizi}>Hapus Data Poli Gizi</button>
                            </div>
                          </div>
                        )
                      ) : (null)
