...
import DataPoliUmum from "@/components/PoliUmum/DataPoliUmum";
...
function useGlobalListPasienPoliUmum() {
  return useSelector((state) => state.listPasienPoliUmum, shallowEqual)
}
const mainPoliUmum = () => {
  ...
  const listPasienPoliUmum = useGlobalListPasienPoliUmum();
  ...
  const cariPasien = () => {
    Swal.showLoading();
    axiosClient.post('Poliumum/cariDataPoliUmum', { search: cariIpt }).then(({ data }) => {

      dispatch({ type: "ISI_DATA_POLI_UMUM", dataPasien: data })
      Swal.close();
    })
    Swal.close();
  }
      ...
      <div className="w-100">
        ...
        <div className="d-block">
          ...
          <div className=" d-inline-block  align-top " style={styleContainer}>
            ...
            <DataPoliUmum listPasien={listPasienPoliUmum} matches={matches} />
          </div>
        </div>
      </div>
    ) : (null)
